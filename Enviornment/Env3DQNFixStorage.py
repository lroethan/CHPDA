...
class Env:
    def __init__(self, workload, candidates, mode):
        ...
        self.tidb_client1 = tihypo.TiDBHypo()
        self.tidb_client2 = tihypo.TiDBHypo()
        ...
        # State info
        self.init_cost = np.array(self.tidb_client1.get_queries_cost(workload)) * self.frequencies
    ...
    def step(self, action):
        ...
        oid = self.tidb_client1.execute_create_hypo(self.candidates[action])
        oids = [oid]
        
        # Calculate storage cost
        storage_cost = self.tidb_client1.get_storage_cost(oids)[0]
        if len(self.candidates[action].split('#')[1].split(',')) == 3:
            storage_cost = storage_cost / 2

        # Calculate current cost and check for improvement
        current_cost_info = np.array(self.tidb_client1.get_queries_cost(self.workload)) * self.frequencies
    ...
    def reset(self):
        ...
        self.current_index_count = 0
        self.current_min_cost = np.array(self.tidb_client1.get_queries_cost(self.workload)).sum()
        ...
        self.tidb_client1.delete_indexes()
        self.last_reward = 0
        self.current_storage_sum = 0
        self.last_perf_gain = sys.maxsize
        self.imp_count = 0
